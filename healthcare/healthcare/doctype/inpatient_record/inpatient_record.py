    ...
    def validate_already_scheduled_or_admitted(self):
        ...
        if ip_record:
            msg = _(
                ...
                + """ <b><a href="/app/Form/Inpatient Record/{0}">{0}</a></b>""".format(
                    ip_record[0].name
                )
            )
            frappe.throw(msg)
        ...
        order = frappe.get_doc(
            {
                ...
                "status": (
                    "draft-Medication Request Status"
                    if medication_request
                    else "draft-Request Status"
                ),
            ...
            order.update(
                {
                    ...
                    "medication_item": (
                        line_item.get("drug_code") if line_item.get("drug_code") else ""
                    ),
                }
            )
        else:
...
def validate_inpatient_invoicing(inpatient_record):
    if frappe.db.get_single_value(
        "Healthcare Settings", "allow_discharge_despite_unbilled_services"
    ):
        return
...
def validate_incompleted_service_requests(inpatient_record):
    ...
    if service_requests and len(service_requests) > 0:
        service_requests = [
            get_link_to_form("Service Request", service_request)
            for service_request in service_requests
        ]
