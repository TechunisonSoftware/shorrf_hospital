...
class MedicationRequest(ServiceRequestController):
    ...
    @staticmethod
    def print_medication_details(patient_name):
        # Fetch Medication Request records for the given patient_name
        medication_requests = frappe.get_all('Medication Request', filters={'patient_name': patient_name}, fields=['patient_name', 'medication_item', 'medication'])
    
        # Loop through and print the details
        for request in medication_requests:
            print(f"Patient Name: {request.patient_name}, Medication Item: {request.medication_item}, Medication: {request.medication}")
