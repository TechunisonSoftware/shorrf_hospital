...
class ServiceRequest(ServiceRequestController):
    ...
    def before_insert(self):
        ...
        if self.amended_from:
            frappe.db.set_value(
                "Service Request", self.amended_from, "status", "revoked-Request Status"
            )
    ...
    def update_invoice_details(self, qty, sales_id):
        ...
        self.db_set(
            {"qty_invoiced": qty_invoiced, "billing_status": status, "sales_invoice": sales_id}
        )
...
def make_lab_test(service_request):
    ...
    if (
        ...
        frappe.throw(
            _("Service Request needs to be invoiced before proceeding"),
            title=_("Payment Required"),
        )
    ...
    if service_request.source_doc == "Patient Encounter":
        doc.patient_encounter = service_request.order_group
